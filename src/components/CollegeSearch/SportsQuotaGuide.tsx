...
import { Trophy, ChevronDown, ExternalLink, Search, X, Shield, MapPin, Hash, Filter, AlertTriangle, Info, GraduationCap } from 'lucide-react';
import { cn } from '@/lib/utils';
import { tneaSportsQuotaColleges, getTotalColleges as getTNEATotal, getDistrictCount, getGovtCount, TNEASportsCollege } from './sportsQuotaData';
import { allCollegesFlat, getFieldCounts, getAllDistricts, AllCollegeFlat } from './allCollegesHelper';
...
export const SportsQuotaGuide = () => {
  ...
  const [verifyCollege, setVerifyCollege] = useState<any>(null);
  const [showInfo, setShowInfo] = useState(false);

  const fieldCounts = useMemo(() => getFieldCounts(), []);
  const districts = useMemo(() => getAllDistricts(), []);

  // Filtered colleges from main database
  const filtered = useMemo(() => {
    let result = allCollegesFlat;
    if (selectedField !== 'All') {
      result = result.filter(c => c.field === selectedField);
    }
    if (search.trim()) {
  ...
  const grouped = useMemo(() => {
    const map: Record<string, AllCollegeFlat[]> = {};
    filtered.forEach(c => {
  ...
  const totalAll = allCollegesFlat.length;
  ...
  return (
    <>
      <div className="rounded-2xl overflow-hidden" style={{ background: '#0f0f14' }}>
        ...
        <div className="relative overflow-hidden p-5 pb-4" style={{ background: 'linear-gradient(135deg, #1a1a24 0%, #0f0f14 50%, #1a1510 100%)' }}>
          ...
          <div className="relative">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <span className="text-xs font-bold px-2.5 py-1 rounded-lg bg-white/5 text-gray-400">16 Fields</span>
            </div>
          </div>
        </div>
        ...
        {showInfo && (
          <div className="px-4 py-3 space-y-2" style={{ background: '#111118', borderBottom: '1px solid #2a2a35' }}>
            <div className="rounded-xl p-3" style={{ background: '#1a1a24', border: '1px solid #2a2a35' }}>
              <div className="space-y-1 text-xs text-gray-400">
                ...
                <p>• Applies to: Engineering, Arts, Medical, Dental, Law, ALL fields</p>
        ...
        <div className="px-3 py-2.5 overflow-x-auto" style={{ background: '#111118', borderBottom: '1px solid #1f1f2a' }}>
          ...
          <div className="flex gap-1.5 min-w-max">
            {fields.map(f => {
              ...
              const count = f.id === 'All' ? totalAll : (fieldCounts[f.id] || 0);
              return (
                <button
                  key={f.id}
                  ...
                  className={cn(
                    ...
                    isActive ? "text-black" : count > 0 ? "text-gray-400 hover:text-white" : "text-gray-600 opacity-40"
                  )}
        ...
        <div className="p-3" style={{ background: '#111118', borderBottom: '1px solid #1f1f2a' }}>
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search college name, course, or district..."
              value={search}
        ...
        {(selectedDistrict !== 'All' || search.trim() || selectedField !== 'All') && filtered.length > 0 && (
          <div className="px-4 py-2 flex items-center justify-between" style={{ background: '#111118', borderBottom: '1px solid #1f1f2a' }}>
            <span className="text-xs text-gray-500">{filtered.length} {selectedField !== 'All' ? fields.find(f => f.id === selectedField)?.label + ' ' : ''}colleges</span>
        ...
        <div className="max-h-[calc(100vh-380px)] overflow-y-auto" style={{ background: '#0f0f14' }}>
          {selectedDistrict === 'All' && !search.trim() && selectedField === 'All' ? (
            <div className="p-10 text-center">
              ...
              <p className="text-xs text-gray-500 mt-1">Choose from the options above to see colleges</p>
              <p className="text-xs mt-3" style={{ color: GOLD }}>{totalAll} colleges across {districts.length} districts, 16 fields</p>
            </div>
          ) : filtered.length === 0 ? (
            <div className="p-10 text-center">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1 flex-wrap">
                            <span className={cn("text-xs font-bold px-1.5 py-0.5 rounded border", tc.bg, tc.text, tc.border)}>
                          ...
                          {college.feeRange && (
                            <p className="text-xs mt-1" style={{ color: GOLD }}>💰 {college.feeRange}</p>
                          )}
                        </div>
                        {college.website && (
                          <a href={college.website.startsWith('http') ? college.website : `https://${college.website}`} target="_blank" rel="noopener noreferrer"
                            className="flex items-center gap-1 px-2 py-1.5 rounded-lg text-xs font-bold flex-shrink-0 mt-1"
        ...
        <div className="px-4 py-3 text-center" style={{ background: '#111118', borderTop: '1px solid #2a2a35' }}>
          <p className="text-xs text-gray-600">All colleges have 5% Sports Quota seats · Verify at official counselling websites</p>
        </div>
      </div>
    </>
  );
