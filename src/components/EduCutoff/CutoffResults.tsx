  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border p-4 md:p-6 animate-fade-in">
      ...
      <div className="flex justify-center pt-2">
        <button
          onClick={() => {
            const msg = showTNEA && result.tneaCutoff
              ? `📊 My TNEA Cutoff Result (VAZHIKATTI)\n\n🎯 TNEA Cutoff: ${result.tneaCutoff}/200\n📝 Overall: ${result.overallPercentage}%\n📈 Percentile: ${result.percentile}%\n\nCalculate yours free 👉 https://horizons-ai-guide-app.vercel.app/edu-cutoff`
              : `📊 My 12th Result Summary (VAZHIKATTI)\n\n📝 Overall: ${result.overallPercentage}%\n📈 Percentile: ${result.percentile}%${result.neetScore ? `\n🏥 NEET Score: ${result.neetScore}/720` : ''}\n\nCheck yours free 👉 https://horizons-ai-guide-app.vercel.app/edu-cutoff`;
            window.open(`https://wa.me/?text=${encodeURIComponent(msg)}`, '_blank');
          }}
          className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#25D366] hover:bg-[#1fba59] text-white font-semibold text-sm shadow-md hover:shadow-lg transition-all hover:scale-[1.02] active:scale-[0.98]"
        >
