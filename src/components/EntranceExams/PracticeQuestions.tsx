...
import { CheckCircle, XCircle, ChevronRight, RotateCcw, Trophy, Clock, BarChart3, ArrowLeft, Zap } from 'lucide-react';
  ...
  const handleNext = () => {
    if (currentQ + 1 >= questions.length) {
      ...
      saveAttempt(examName, {
        ...
        pct: Math.round(((score + (selected === q.answer ? 1 : 0)) / questions.length) * 100),
        timeSeconds: elapsed,
  ...
  if (showHistory) {
    return (
      <div className="space-y-4">
        ...
        <h3 className="text-sm font-bold text-gray-800">📊 Your Past Attempts — {examName}</h3>
        {history.length === 0 ? (
          <p className="text-xs text-gray-500 text-center py-8">No attempts yet. Complete a quiz to see your history.</p>
        ) : (
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {Object.entries(h.subjectBreakdown).map(([sub, data]) => (
                      <span key={sub} className={cn("text-[10px] px-2 py-0.5 rounded-full font-medium",
                        data.correct / data.total >= 0.7 ? 'bg-emerald-50 text-emerald-700' : 
                        data.correct / data.total >= 0.4 ? 'bg-amber-50 text-amber-700' : 'bg-red-50 text-red-700'
                      )}>
                        {sub}: {data.correct}/{data.total}
                      </span>
                    ))}
                  </div>
                )}
    ...
    return (
      <div className="space-y-4">
        ...
        <div className={cn("rounded-2xl p-6 text-center border-2",
          ...
          <p className="text-xs text-gray-500 mt-1">⏱ Time: {formatTime(elapsed)} · {examName}</p>
          <p className="text-sm font-medium text-gray-700 mt-2">
            {pct >= 80 ? '🎉 Excellent! You\'re well prepared.' : pct >= 60 ? '👍 Good! Focus on weak areas below.' : pct >= 40 ? '📚 Keep practicing. Review the explanations.' : '💪 Don\'t give up. Start with NCERT basics.'}
          </p>
        </div>

        {/* Subject breakdown */}
        <div>
          <p className="text-sm font-bold text-gray-800 mb-2">Subject-wise Performance</p>
          <div className="space-y-2">
              ...
              return (
                <div key={sub} className="bg-white rounded-xl p-3 border border-gray-200">
                  ...
                  <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                    <div className={cn("h-full rounded-full transition-all", sp >= 70 ? 'bg-emerald-500' : sp >= 40 ? 'bg-amber-500' : 'bg-red-500')} style={{ width: `${sp}%` }} />
                  </div>
        ...
        <div className="flex gap-2">
          <button onClick={restart} className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-gray-900 text-white text-sm font-bold hover:bg-gray-800">
            <RotateCcw className="w-4 h-4" /> Try Again
          </button>
          <button onClick={() => setShowHistory(true)} className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-white border-2 border-gray-200 text-gray-700 text-sm font-bold hover:border-gray-400">
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="text-xs font-bold text-gray-600">{currentQ + 1}/{questions.length}</span>
          <div className="w-24 h-1.5 bg-gray-200 rounded-full overflow-hidden">
            <div className="h-full bg-emerald-500 rounded-full transition-all" style={{ width: `${((currentQ + 1) / questions.length) * 100}%` }} />
          </div>
        </div>
      ...
      {showAnswer && (
        <div className={cn("rounded-xl p-4 border-2", selected === q.answer ? 'bg-emerald-50 border-emerald-200' : 'bg-red-50 border-red-200')}>
          <p className="text-xs font-bold text-gray-700 mb-1">{selected === q.answer ? '✅ Correct!' : '❌ Wrong — Correct answer: ' + q.options[q.answer]}</p>
      ...
      {showAnswer && (
        <button onClick={handleNext}
          className="w-full flex items-center justify-center gap-2 py-3.5 rounded-xl bg-gray-900 text-white text-sm font-bold hover:bg-gray-800 transition-all active:scale-[0.98]">
