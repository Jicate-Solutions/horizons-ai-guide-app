          ...
          <form onSubmit={handleSubmit}>
            <CardContent className="space-y-4">
              ...
              <button
                ...
                onClick={async () => {
                  const { error } = await supabase.auth.signInWithOAuth({
                    provider: 'google',
                    options: { redirectTo: window.location.origin + (redirectParam || '/career-assessment/colleges') },
                  });
                  if (error) toast({ title: 'Google Sign-In Failed', description: error.message, variant: 'destructive' });
                }}
                className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl border-2 border-gray-200 bg-white hover:bg-gray-50 text-gray-700 font-semibold text-sm transition-all hover:shadow-md active:scale-[0.98]"
              >
              ...
              <div className="relative">
                ...
                <div className="relative flex justify-center text-xs"><span className="bg-white px-3 text-gray-400 font-medium">or use mobile number</span></div>
              </div>
